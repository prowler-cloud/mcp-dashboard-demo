...
Copy everything below the line into Claude Code (or any Claude client with
the Prowler MCP server enabled). Replace the values in the **CUSTOMIZE FOR
ME** section before submitting, or leave them blank and Claude will ask.
...
- EVERY type card opens a POPOVER on click (single- and multi-account alike,
  ...
  anchor right), containing per-type "All"/"None" STATE controls (lit emerald exactly like
  the panel-level pair: All lit when every account of that type is selected,
  None when zero, neither on partial) and the account chips. The popover stays open while toggling chips inside it; clicking
...
- **Default head branch:** `<fix/branch-name>`
- **Default PR labels:** `<label1>, <label2>`

If you don't know my values, ask once at the start.
